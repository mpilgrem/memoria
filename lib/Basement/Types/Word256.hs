...
import           Basement.Compat.NumLiteral ( HasNegation (..), Integral (..) )
import           Basement.Compat.Primitive ( bool# )
import           Basement.Numerical.Conversion
                   ( intToInt64, int64ToWord64, wordToInt, word64ToWord )
...
import           GHC.Prim
                   ( eqWord#, plusWord#, plusWord2#, wordToWord64#
                   , word64ToWord#
                   )
import           GHC.Word ( Word64 (..) )
import qualified Prelude (fromInteger, Num (..), quot, rem, mod)
import           Numeric.Natural ( Natural )
import           Prelude
                   ( Applicative (..), Bool (..), Bounded (..), Enum (..)
                   , Eq (..), Integer, Ord (..), Ordering (..), Maybe (..)
                   , Show (..), ($), (.), (&&), (||), (<$>)
                   , otherwise
                   )
...
fromNatural n = Word256
    (Prelude.fromInteger (toInteger n `Bits.unsafeShiftR` 192))
    (Prelude.fromInteger (toInteger n `Bits.unsafeShiftR` 128))
    (Prelude.fromInteger (toInteger n `Bits.unsafeShiftR` 64))
    (Prelude.fromInteger $ toInteger n)
