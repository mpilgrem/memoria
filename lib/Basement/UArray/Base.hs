...
import qualified Basement.Alg.Class as Alg ( Indexable )
import           Basement.Bindings.Memory
                   ( sysHsMemcmpBaBa, sysHsMemcmpBaPtr, sysHsMemcmpPtrBa
                   , sysHsMemcmpPtrPtr
                   )
import           Basement.Block.Base ( Block (..), MutableBlock (..) )
import qualified Basement.Block.Base as BLK ( unsafeIndex, unsafeThaw )
import qualified Basement.Block.Base as MBLK
import qualified Basement.Compat.ExtList as List ( length )
import           Basement.FinalPtr
                   ( FinalPtr, withFinalPtr, withUnsafeFinalPtr )
...
onBackendPure :: (Block ty -> a)
              ...
              -> UArray ty
              -> a
onBackendPure goBA goAddr = onBackend goBA (\_ -> pureST . goAddr)
